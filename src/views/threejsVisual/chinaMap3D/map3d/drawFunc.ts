...
import { GeoJsonType, GeoJsonFeature, GeometryCoordinates, GeometryType, ExtendObject3D } from "./typed";
import { ProjectionFnParamType } from ".";
import { mapConfig } from "./mapConfig";

export function getDynamicMapScale(mapObject3D: THREE.Object3D, containerRef: any) {
	...
	const scale = Math.round(Math.sqrt(refArea / (size.x * size.y * 400))) + parseFloat((Math.random() + 0.5).toFixed(2));
	return scale;
}

// 绘制挤出的材质
export function drawExtrudeMesh(point: [number, number][], projectionFn: any): any {
...
// 生成地图3D模型
export function generateMapObject3D(mapdata: GeoJsonType, projectionFnParam: ProjectionFnParamType) {
	// 地图对象
	...
	basicFeatures.forEach((basicFeatureItem: GeoJsonFeature) => {
		...
		// 每个中心点位置
		const featureCenterCoord: any = basicFeatureItem.properties.centroid && projectionFn(basicFeatureItem.properties.centroid);
		// 名字
